...
while (my $line=<IN>) {
    ...
    my $gfp = "SSHT";
    my $nm = &num_mutation($gfp, $name);

    push(@$tbl, [$name, $nm, @feat, $pred]);
}
close(IN);
...
sub num_mutation {
    my ($str1, $str2) = @_;
    length($str1)==length($str2) or die "wrong sequence length\n";

    my $nm = 0;
    for (my $i=0; $i<length($str1); $i++) {
	substr($str1, $i, 1) eq substr($str2, $i, 1) or $nm++;
    }
